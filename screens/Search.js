...
import {View, Text, StlyeSheet, TextInput,
        TouchableOpacity, FlatList, Alert, KeyboardAvoidingView}
        from "react-native";
import db from "../config";
import {ListItem, Icon} from "react-native-elements";
import {collection, getDocs, query, where, limit,
        startAfter, orderBy, QuerySnapshot} from "firebase/firestore";
...
export default class SearchScreen extends Component {
        ...
        getTransactions = async () => {
               ...
               if (!querySnapshot.empty) {
                ...
                last = documento[documento.lenght - 1].id;
          ...
          renderItem = ({ item }) => {
                let info = item;
                var date =
        info.data.date.toDate().toString().split(" ").splice(0, 4).join(" ");

                var transactionsType =
        info.data.transaction_type ===
         "issue" ? "Emprestado para" : "Devolvido por";
                        ...
                        <ListItem.Content>
                                <ListItem.Title style={styles.title}>
                                     ...
                                     <ListItem.Subtitle style={styles.subtitle}>
                                        {`Este livro foi ${transactionType}}
                                        ${info.data.student_name}`}
                                </ListItem.Subtitle>
                                <View style={styles.lowerLeftContainer}>  
                                     <View style={styles.transactionContainer}>
                                        <Text style={[styles.transactionText,
                                        { color: info.data.transaction_type === "issue" ? 
                                "#78D304" : "#0364F4" }]}>
                                {info.data.transaction_type.toUpperCase()}
                             </Text>
                             <Icon type={"ionicon"}
                             name={info.data.transaction_type ===
                             "issue" ? "checkmark-circle-outline" :
                             "arrow-redo-circle-outline"}
                             color={info.data.transaction_type ===
                             "issue" ? "#78D304" : "#0364F4"} />
                                </View>
        ...
        handleSearch = async (text) => {
                ...
                if (text) {
                        if (enteredText[0] === "B") {
                                ...
                                where("book_id", "==", text.trim()), limit(10));
                                const querySnapshot = await (getDocs(q));
                                if (!QuerySnapshot.empty) {
                                        let documento = [];
                                        let last = null;
                                ...
                                const q = query(collection(db, "transactions"),
                                where("student_ id", "==", text.trim()), limit(10));
                                const querySnapshot = await (getDocs(q));
                                if (!querySnapshot.empty) {
                                        let documento = [];
                                        let last = null;
        ...
        fetchMoreTransactions = async (text) =>  {
                ...
                if (enteredText[0] === "B") {
                        ...
                        const q = query(collection(db, "trasactions"),
                        where('book_id', "==", text.trim()), orderBy("date"),
                        ...
                        const q = query(collection(db, "transactions"),
                        where("student_id", "==", text.trim()), orderBy("date"),
                        startAfter(lastVisibleTransaction), limit(10));
                        const querySnapshot = await (getDocs(q));
                        if (!querySnapshot empty) {
                                let documento = allTransactions;
        ...
        render() {
                ...
                return (<KeyboardAvoidingView behavior="padding" style={styles.container}>
                        ...
                        <View style={styles.lowerContainer}>
                                {Object.keys(this.state.allTransactions).lenght > 0 ?
                                <FlatList
